...
/**
 * PushManager 为对外接口, 所有调用均是通过 PushManager.getInstance() 日志过滤器输入 PushManager,
 * 在接口调用失败会有对应 Error 级别
 * log 提示.
 */
public class GetuiSdkDemoActivity extends Activity implements OnClickListener {

    private static final String TAG = "GetuiSdkDemo";
    private static final String MASTERSECRET = "uzIkLr5Irc7GZOzvze2l19";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        appKeyView.setText(String.format("%s",
            getResources().getString(R.string.appkey) + appkey));
        appSecretView.setText(String.format("%s",
            getResources().getString(R.string.appsecret) + appsecret));
        masterSecretView.setText(String.format("%s",
            getResources().getString(R.string.mastersecret) + MASTERSECRET));
        appIdView.setText(String.format("%s",
            getResources().getString(R.string.appid) + appid));
        ...
        // 注册 intentService 后 PushDemoReceiver 无效, sdk 会使用 DemoIntentService
        // 传递数据,
        // AndroidManifest 对应保留一个即可(如果注册 DemoIntentService, 可以去掉
        // PushDemoReceiver, 如果注册了
        // IntentService, 必须在 AndroidManifest 中声明)
        PushManager.getInstance().registerPushIntentService(
            this.getApplicationContext(), DemoIntentService.class);
        ...
        // cpu 架构
        Log.d(TAG, "cpu arch = " + (Build.VERSION.SDK_INT < 21 ? Build.CPU_ABI
        ...
        if (requestCode == REQUEST_PERMISSION) {
            if ((grantResults.length == 2
                ...
                PushManager.getInstance()
                    .initialize(this.getApplicationContext(), userPushService);
            } else {
                Log.e(TAG,
    ...
    public void onDestroy() {
        ...
        DemoApplication.payloadData.delete(0,
            DemoApplication.payloadData.length());
        super.onDestroy();
    }
    ...
    public void onClick(View v) {
        if (v == clearBtn) {
            ...
            if (isServiceRunning) {
                ...
                PushManager.getInstance()
                    .stopService(this.getApplicationContext());
    ...
    private void addTag() {
        ...
        alertBuilder.setTitle(R.string.add_tag)
            ...
            .setPositiveButton(R.string.confirm,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        int i = PushManager.getInstance().setTag(context,
                            tagParam, "" + System.currentTimeMillis());
    ...
    private void getVersion() {
        ...
        Toast.makeText(this,
            getResources().getString(R.string.show_version) + version,
            Toast.LENGTH_SHORT).show();
    }

    private void getCid() {
        String cid = PushManager.getInstance().getClientid(this);
        Toast.makeText(this, getResources().getString(R.string.show_cid) + cid,
            ...
            if (info != null) {
                for (NetworkInfo ni : info) {
                    if (ni.getState() == NetworkInfo.State.CONNECTED) {
                        Log.d(TAG, "type = " + (ni.getType() == 0 ? "mobile"
                            : ((ni.getType() == 1) ? "wifi" : "none")));
                        return true;
                    }
                }
            }
    ...
    private void setSilentime() {
        final View view = LayoutInflater.from(this)
            .inflate(R.layout.silent_setting, null);
        AlertDialog.Builder alertBuilder = new AlertDialog.Builder(this);
        alertBuilder.setTitle(R.string.set_silenttime)
            .setNegativeButton(R.string.cancel,
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        try {
                            int beginHour = Integer
                                .valueOf(String.valueOf(beginText.getText()));
                            int durationHour = Integer.valueOf(
                                String.valueOf(durationText.getText()));

                            boolean result = PushManager.getInstance()
                                .setSilentTime(context, beginHour,
                                    durationHour);

                            if (result) {
                                Toast
                                    .makeText(context,
                                        "begin = " + beginHour + ", duration = "
                                            + durationHour,
                                        Toast.LENGTH_SHORT)
                                    .show();
                                Log.d(TAG, "setSilentime, begin = " + beginHour
                                    + ", duration = " + durationHour);
                            } else {
                                Toast.makeText(context,
                                    "setSilentime failed, value exceeding",
                                    Toast.LENGTH_SHORT).show();
                                Log.d(TAG,
                                    "setSilentime failed, value exceeding");
                            }
    ...
    private void unBindAlias() {
        ...
        new AlertDialog.Builder(GetuiSdkDemoActivity.this)
            .setTitle(R.string.unbind_alias).setView(editText)
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (alias.length() > 0) {
                            PushManager.getInstance().unBindAlias(
                                GetuiSdkDemoActivity.this, alias, false);
                            Log.d(TAG, "unbind alias = "
    ...
    private void bindAlias() {
        final EditText editText = new EditText(GetuiSdkDemoActivity.this);
        new AlertDialog.Builder(GetuiSdkDemoActivity.this)
            .setTitle(R.string.bind_alias).setView(editText)
            .setPositiveButton(android.R.string.ok,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (editText.getEditableText() != null) {
                            String alias = editText.getEditableText()
                                .toString();
                            if (alias.length() > 0) {
                                PushManager.getInstance().bindAlias(
                                    GetuiSdkDemoActivity.this, alias);
                                Log.d(TAG, "bind alias = "
                                    + editText.getEditableText().toString());
                            }
                        }
                    }
    ...
    private void showNotification() {
        if (isNetworkConnected()) {
            ...
            param.put("pushTitle",
                getResources().getString(R.string.push_notification_title)); // pushTitle请填写您的应用名称
            ...
            linkMsg.put("linkMsgTitle",
                getResources().getString(R.string.push_notification_msg_title)); // 推送消息的标题
            linkMsg.put("linkMsgContent", getResources()
                .getString(R.string.push_notification_msg_content)); // 推送消息的内容
    ...
    private void showTransmission() {
        if (isNetworkConnected()) {
            ...
            param.put("data",
                getResources().getString(R.string.push_transmission_data));
            // 当前请求时间，可选
            param.put("time",
